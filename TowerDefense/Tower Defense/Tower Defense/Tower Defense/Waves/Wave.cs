...
namespace Tower_Defense
{
    class Wave
    {
        ...
        public static int[][] enemyTypeSpawned = new int[27][];
        ...
        private void AddRedEnemy()
        {
            Enemy Redenemy = new Enemy(enemyTexture[0],
                                level.Waypoints.Peek(), enemyHealth, enemyBounty, 1f);  //Health, Bounty, Speed
        ...
        private void AddBlueEnemy()
        {
            Enemy Blueenemy = new Enemy(enemyTexture[1],
                                level.Waypoints.Peek(), enemyHealth * 2 * (waveNumber / 2), enemyBounty, 1.25f); 
        ...
        private void AddGreenEnemy()
        {
            Enemy Greenenemy = new Enemy(enemyTexture[2],
                                level.Waypoints.Peek(), enemyHealth * 3 * waveNumber, enemyBounty, 1.50f); 
        ...
        private void AddYellowEnemy()
        {
            Enemy Yellowenemy = new Enemy(enemyTexture[3],
                                level.Waypoints.Peek(), enemyHealth * 4 * (waveNumber * 2), enemyBounty, 1.75f); 
        ...
        private void AddBlackEnemy()
        {
            Enemy Blackenemy = new Enemy(enemyTexture[4],
                                level.Waypoints.Peek(), enemyHealth * 8 * (waveNumber * 4), enemyBounty, 2f);
            ...
            for (int i = 0; i < enemies.Count; i++)
            {
                ...
                if (enemy.IsDead) //Check if the enemy has died
                {
                    if (enemy.CurrentHealth > 0) // Enemy is at the end
                    {
                        ...
                        switch ((int)enemy.StartHealth) //Find out which enemy it is based on their starting health
                        {
                            case 50: { player.Lives -= 1; break; }
                            case 100: { player.Lives -= 2; break; }
                            case 150: { player.Lives -= 3; break; }
                            case 200: { player.Lives -= 4; break; }
                            case 500: { player.Lives -= 5; break; }
                        }
                    }
